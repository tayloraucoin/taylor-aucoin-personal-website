...
export const consciousConnections = {
  ...
  atAGlance: [
    {
      label: "Engagement",
      value:
        "My own product — a passion project built between client engagements, in the spare cycles",
    },
    {
      label: "Product",
      value:
    ...
    },
    {
      label: "AI system",
      value:
        "A compress → reason pipeline — Haiku distills, Sonnet coaches — with a safety classifier reading raw text in parallel",
    },
    {
      label: "Safety floor",
      value:
    ...
    groups: [
      {
        ...
        bullets: [
          ...
          "No streaks, no gamification, no engagement bait — and the session itself is never paywalled; conversion happens in the calm",
        ],
      },
      {
        ...
        bullets: [
          ...
          "One person carries that floor alongside product, design, the AI pipeline, and billing — so the system is designed to not depend on me being careful",
        ],
      },
    ],
  },
    ...
    sections: [
      {
        ...
        bullets: [
          ...
          "A written precedence hierarchy decides which document wins when docs disagree — conflicting instructions get a defined answer, not a silent coin flip",
      ...
      },
      {
        ...
        bullets: [
          ...
          "Row-level security enabled deny-by-default on every table, with policy factories so each table's access intent is declared, not improvised",
          "Every request runs through a transaction-scoped RLS bridge that sets the user's identity as Postgres session variables — the database enforces who sees what, not the service code",
      ...
      },
      {
        ...
        bullets: [
          ...
          "Coach Privately is a walled channel: its context loader cannot see the partner's private material — absent by construction, not filtered by prompt",
      ...
      },
      {
        ...
        bullets: [
          ...
          "Golden-set eval harnesses are written before their prompts and tracked as open gates until they run — a prompt's behavior counts as designed, not measured, until then",
        ],
      },
    ],
  },
  decisions: [
    ...
    },
    {
      ...
      why: "The codebase is built AI-assisted, and a pre-1.0 ORM drifting under a model that emits last year's syntax is a failure mode I'd already met, not a hypothetical. The pin plus a written syntax contract means every agent thread writes against one known API. The cost: upgrades are deliberate events with human review, never casual bumps.",
    },
  ],
  built: {
    intro:
  ...
  broke: {
    ...
    categories: [
      ...
      },
      {
        ...
        body: "The compression prompt's own binding law said never drop a need or a repair bid — and the output schema had no fields for either, so the model produced them and validation threw them away. The quiet drift between prompt and contract that only a code-verified prompt inventory surfaces; found in exactly that audit, then reconciled.",
      },
    ],
    closing:
      "The staff-engineer review role earns its seat the same way: its pass over the private-threads slice found two blocking defects — a tab bounce that dropped someone into the wrong conversation, and a drawer-wide soft-lock — both fixed before the slice closed. The roster isn't a gimmick; it's where a solo builder's code review comes from.",
  },
  outcome: {
    intro:
