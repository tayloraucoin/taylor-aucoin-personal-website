...
import { hexToRgba, INTERACTIVE, type Pad, type Seg } from "./types";

/**
 * FROZEN — the field as it shipped before the banner rework, transplanted
 * verbatim from commit 260649a ("Fix mobile animation…"). It exists so
 * `FIELD_VERSION` in lib/config.ts can A/B the old page field against the
 * reworked one; the rework's page GROWTH params claim to reproduce this
 * exactly, and this file is the ground truth for that claim.
 *
 * Do not edit or "converge" it — any change here quietly invalidates the A/B.
 * Page variant only: this predates the banner, the control handle, and the
 * fixed-size/forced-dpr options. Delete the file (and the toggle) when the
 * A/B has settled.
 */
export function mountRootFieldV1(
