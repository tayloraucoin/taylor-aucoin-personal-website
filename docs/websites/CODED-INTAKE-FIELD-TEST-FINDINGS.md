...
**Correction, 2026-09-14.** "Both projects" was wrong. A read-only bucket listing with each tier's service-role key showed production with `PUBLIC` · `PRIVATE` · `intake`, and staging — the project local borrows — with `public` only. Taylor hit the identical `The related resource does not exist` line on localhost on 2026-09-14 on project images, the step-8 portrait, and the voice note (which uploads to the bucket before it can be transcribed, so OpenAI was never reached). Reopened for staging as **PORT-H5** (`specs/PORT-H5-storage-buckets-by-tier.md`), which also carries the audit of every upload site and its bucket path, and adds `yarn storage:verify` so this is a one-line check next time.

**Closed on staging 2026-09-14.** Taylor created `intake` and `PRIVATE` by hand in the staging project (matching production's dashboard-created pair), same day the gap was found. `yarn storage:verify` confirms both tiers now hold `intake` (private) and `PRIVATE` (private) with matching flags — the intake-process bucket set fully mirrors production. A separate, pre-existing casing mismatch surfaced by the same script: production has no bucket named `public` (only `PUBLIC`), so `captureBucket()`'s default resolves to a bucket that does not exist there — the taste-gallery capture path is broken in production unless `SUPABASE_LIVE_PUBLIC_BUCKET=PUBLIC` is set on Vercel. This is the Phase 10 open item (`00-build-order.md`), now confirmed rather than suspected; it does not touch client uploads or the voice note and was left unset here — it needs a Vercel environment variable, which this machine cannot set.
