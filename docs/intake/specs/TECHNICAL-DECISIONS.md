...
## M-INT-23 · The catalogue lives in `products`; Stripe ids and amounts are written by the catalogue script in one motion

**Decision (2026-08-21).** Sellable things are rows: `products` carries key, kind, client-facing copy, `price_cents`, `offered_at_checkout`, and the Stripe product/price ids. `yarn db:seed` owns identity, copy, amounts, and tier-aware Stripe ids (fixed UUIDs, idempotent upserts — production gets live-mode ids, staging/local get test-mode ids). `yarn stripe:catalogue --apply` is the tool that talks to Stripe when a price actually changes: it mints the new Price, archives the old one, and prints the id to paste into the seed's map. Purchases are recorded per engagement on `engagement_products` at session creation (amounts copied from the live price — the deal as struck) and stamped paid by the settlement path. Mirrors CC's `subscription_plans` (`price_cents` + `stripe_price_id` on one row).

**Alternatives rejected.** Env-var price ids only (no analytics, no P0 list to render, per-tier env sprawl); reading amounts from Stripe on every P0 render (N calls per view; the misquote guarantee instead moved to session creation, which verifies each Price against Stripe and fails closed); making `/websites` read the DB (a static marketing page gains a database dependency to render prose it owns — price changes are rare and run through one runbook).

**Price-change runbook.** Edit `scripts/setup-stripe-catalogue.ts` → `yarn stripe:catalogue --apply` (per tier) to mint the new Price and learn its id → paste the id into `STRIPE_CATALOGUE_IDS` in `scripts/seed-products.ts` → `yarn db:seed` against that tier's database → update `content/websites.ts` and the how_we_work PDF → if the terms' numbers changed, bump `TERMS_VERSION`.

## 2026-09-12 · M-INT-24 · A step detects its own stale render and refreshes, rather than every link opting out of prefetch

**Context (as it was then):** The first client through the coded track left notes and ratings on the taste step, went back to the work step for several minutes, pressed Continue, and saw the taste step without them; a reload brought them back. The database had them the whole time. Traced in the installed router (Next 15.5.23): every step route is dynamic with no `loading.tsx`, so a `<Link>` prefetch returns the route tree with no rendered components; a navigation that uses such an entry for the first time leaves the client cache untouched (`applyFlightData` returns `false` on null seed data). The first visit fetches lazily and that node is retained; prefetch entries expire after five minutes, so a return after that mints a new entry, takes the same first-use path, and renders the retained node with no request. Browser back/forward restores from the cache regardless of age. Reproduced on staging with the refresh disabled, and gone with it enabled.
**Options weighed:** A) `prefetch={false}` on every link into a step, plus a `popstate` refresh for back/forward. B) A `loading.tsx` on the step route, which makes the prefetch carry seed data and the lazy fetch happen every time. C) Stamp each server render of the shell with an id, mount a client guard that treats the same id at the same pathname after a navigation away as a replayed render, and `router.refresh()`; key the step body by the id so the refresh re-seeds the form.
**Decision:** C — `app/websites/intake/_components/refresh-if-stale.tsx`, mounted by `StepShell` (both tracks) and the coded review page. A is a promise every future link must keep and still misses back/forward. B is a visible loading state between steps that no spec chose, and the footer's save indicator would blink out on every transition. C is one mechanism in one place; a link added anywhere is covered. The detector needs no clock and no storage: module state lives exactly as long as the router cache it guards against.
**Consequences:** `main` in `StepShell` is keyed by the render id, so *every* `router.refresh()` on a step now remounts the form — including the in-step ones from `step-proposals`, `source-list`, and `step-ingest`, which had assumed a refresh re-seeds `useStepAutosave`'s `initial` and were wrong (`useState` reads it once). Nothing typed is lost to a remount: an unsaved edit is already in localStorage and rehydrates on mount; a saved one is in `initial`. A replayed render is visible for the round trip before the refresh lands. Development's double-mount is excluded by requiring a pathname change between the two sightings.
**Revisit trigger:** A Next upgrade whose router fills the cache from a null-seed prefetch, or the segment cache (`experimental.clientSegmentCache`) becoming the default — either makes the guard a no-op that can be deleted, not a bug.
