...
2026-09-25 · REV-4 · On staging, `0018` failed: one pre-REV-4 round existed ("Phase 1 · local test", created 2026-09-23 by the removed CLI; 0 comments, 0 submissions, unlinked), so the NOT NULL `client_app` could not be added, and `0019`–`0020` never ran. drizzle-kit printed no error — the run stops mid-spinner without "migrations applied successfully" · remedy: delete that round, re-run `yarn db:migrate`; check `select count(*) from review_rounds` is 0 before migrating production.
