...
2026-09-12 · FIN-1 · Review fixes to `recordOrder`: (a) `status` no longer regresses to `open`/`failed` on a row with `paid_at` set — Stripe delivers out of order and an auto-charged invoice fires `finalized` and `paid` together; (b) a metadata id naming no engagement records unlinked as the spec's edge state says, instead of falling through to the email match; (c) `linkBasketRows` / `reconcileEngagement` run only on a `metadata` link · (b)+(c) together closed a path where a deleted engagement's session could stamp a basket row and `terms_accepted_at` on the same address's other engagement.
