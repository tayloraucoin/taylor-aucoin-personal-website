...
- **`recordOrder(input)` takes a discriminated union — `{ kind: "checkout", session, lineItems }` or `{ kind: "invoice", invoice }` — and returns the `orders` row.** Upsert on `stripe_object_id`. Every column is written as `coalesce(existing, incoming)` except `status` and `updated_at`, which take the incoming value: status is Stripe's fact and may legitimately move (open → paid → refunded). **Amended in review, 2026-09-12:** an incoming `open` or `failed` never lands on a row whose `paid_at` is set — Stripe does not order deliveries, and an auto-charged invoice fires `finalized` and `paid` within a second. Logged.
- **Basket rows and the engagement's fill-nulls are written only when the link came from the site's own metadata.** An email or manual link names the client, not the basket that was charged; stamping it would be a fabricated purchase. (Review, 2026-09-12.)
