# FIN-6 — The add-ons page under the token: a paid client buys any of their track's add-ons, and extra pages by count, in one Checkout
...
**Status:** Not started — D-FIN-1 ratified 2026-09-11 (add-ons **and extra pages**); M-FIN-5 governs the page-count row.
...
A client whose build is bought opens `/websites/coded/intake/<token>/extras` (or the durable tree's twin) and sees the add-ons of their track they have not bought — the pay screen's own set, priced from the catalogue — plus one row for extra pages with a page count, ticks any number, and pays once on hosted Checkout. They return to the same page with the bought rows confirmed, receive Agora's PDF invoice for exactly what they bought, and Taylor gets one ops email. The build's `paid_at` is never touched. What this slice does not do: sell the care plan, sell extra pages a second time from the page (M-FIN-5 — that goes through Taylor), or email anyone an offer (FIN-7).

## Why / intent

- **Taylor, 2026-09-11** — "send a client an email that brings them to a page where they can choose any selection of upsells… specific to which base… maybe another child path." And, later the same day: "allow for the purchase of extra pages from this add-upsells page."
- **M-FIN-5** — extra pages: a quantity stepper (1–20) while unowned, "N pages added" once owned, no second self-serve purchase.
...
- **What this slice is NOT (binding):** no `offeredAtCheckout: false` row on the page **except the track's extra-page product** (SEO post, care plan, and the negotiated build rows stay off); no promo interaction; no quantity input on any row but extra pages; no change to `createDepositCheckout` or the pay screen; no automatic email.
...
- **`createExtrasCheckout(engagement, token, items: { key, quantity }[])` in `deposit.ts`.** Refuses unless `paid_at` is set, every key is either an active `offeredAtCheckout` add-on of `engagement.track` or that track's extra-page key, none is owned, `items.length >= 1`, and quantity is 1 for every row but extra pages, where it is an integer 1–20. Replaces this engagement's unpaid rows for those products (never stacks); the extra-page row is written with `quantity` and per-page `amount_cents` exactly as `createExtraPageCheckout` writes it. Metadata `{ engagement_id, charge_kind: "extras", product_keys: keys.join(","), pages: <n or absent> }`. Success URL the extras page with `?added=1`, cancel with `?canceled=1`. Prices verified against Stripe before a session exists. Logged.
...
The page in the step grammar: eyebrow `ADD TO YOUR BUILD` `[COPY — draft]`, one line saying the site is finished without any of these, then the rows — name, the "How it works" disclosure the pay screen already has (`content/addon-details.ts`), price, a checkbox. The extra-pages row carries a page-count stepper (1–20) beside its per-page price and shows the line total; ticked with a count, it is one line item with that quantity. Owned rows render as confirmed, unticked, no checkbox; an owned extra-pages row reads "N pages added". Below: the total of what is ticked and one `GradientButton` `Pay $N` (disabled at zero), with the trust clause beside it. Press → opening state → Checkout. Return paid → the rows confirmed, one quiet confirmation line. Return cancelled → the page as it was with P0's line verbatim: *"No charge was made. Whenever you're ready."*
...
- **Only `offeredAtCheckout` rows of the engagement's track, plus that track's extra-page product**, resolved from the catalogue by `kind`/`key`, never a hardcoded list of names.
...
**Validators:** `lib/validators/extras.ts` — `token`, `items` (non-empty array of `{ key, quantity }`, keys deduplicated, max 12, quantity an integer 1–20 and exactly 1 unless the key is an extra-page key).
...
1. A paid showcase engagement owning nothing sees the six showcase `offeredAtCheckout` add-ons plus the extra-pages row with a stepper, and no others; a durable engagement sees its own track's set plus its extra-pages row.
...
5. A forged action call with an owned key, an unknown key, an `offeredAtCheckout: false` non-page key, a quantity above 1 on a non-page row, a page count of 0 or 21, or on an unpaid engagement → refused before any Stripe call, each. *(Forge.)*
5a. Three extra pages ticked with two add-ons → one Checkout with three line items, the page line at quantity 3; settlement stamps the page row with `quantity: 3` and the invoice lists `Extra pages × 3`. An engagement that already owns extra pages sees "N pages added" and no stepper; a forged call for the page key is refused. *(Forge.)*
...
- **A second extra-pages purchase from the page** — M-FIN-5; Taylor's CLI or a dashboard invoice.
- **Care plan, balance** — RUNBOOK, Taylor-initiated, unchanged.
...
> Build **FIN-6 — The add-ons page** (attached spec). **`paid_at` is never written here; only the track's `offeredAtCheckout` rows plus its extra-page row with a 1–20 count; refuse owned rows before Stripe; bought state is the basket, never the URL; no urgency near a price; the durable track ships unchanged.**
